...
import { GetAlertDailySummaryQuery } from '../../../domain/model/queries/get-alert-daily-summary.query';
import { GetAlertsQuery } from '../../../domain/model/queries/get-alerts.query';
...
export class AlertQueryServiceImpl implements AlertQueryService {
  ...
  handleGetAlerts(query: GetAlertsQuery, status?: AlertStatus[]): Observable<AlertPage> {
    return this.gateway
      .getAlerts(query.page, query.size, status)
  ...
  handleGetDailySummary(query: GetAlertDailySummaryQuery): Observable<DailyAlertCount[]> {
    return this.gateway
      .getCurrentUserDailyAlertSummary(query.days)
